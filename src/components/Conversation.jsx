...
function Conversation({ contact, chat, setChat }) {
  ...
  const handleSend = (e) => {
    ...
    const newChat = {
      ...chat,
      msgs: [...chat.msgs, { msg: msg, author: contact.display }],
    };
    setChat(newChat);
    ...
    newFriends = newFriends.filter(
      (ch) => ch.usr1 !== chat.usr1 && ch.usr2 !== chat.usr2,
    );
    console.log(newFriends);

    newFriends.push(newChat);
    console.log(newFriends);
  ...
  return contact ? (
    <div className="flex-1 2xl:min-w-[550px] sm:min-w-lg-[700px] flex flex-col h-screen sm:flex hidden">
      <div className="flex justify-center items-center bg-[var(--gray-800)] gap-[14vw] py-[2vh] 2xl:mr-[15px] sm:hidden">
        <Back className="dark:brightness-200 w-[32px] " />
      ...
      <div className="flex justify-between 2xl:mt-[41px] sm:mt-lg-[41px] mt-[2.5vh] 2xl:ml-[56px] sm:ml-lg-[56px] 2xl:pb-[24.5px] pb-lg-[24.5px] 2xl:mr-[54.5px] sm:mr-lg-[54.5px] border-b-2 border-[var(--gray-500)] dark:border-[var(--gray-100)]">
        <div className="flex">
          <img
            src={contact?.image || ProfilePic}
            className="2xl:w-[85px] w-lg-[85px] 2xl:h-[85px] h-lg-[85px] rounded-lg sm:ml-0 ml-[7vw]"
          ></img>
      ...
      <MessageList
        chat={chat}
        curDisplay={chat.usr1 === contact.display ? chat.usr1 : chat.usr2}
      />
      <form
